...
void makeTriangulation(bool delaunay, bool unwantedTriangles)
{
	if (points.size() > 0)
	{
		...
		Triangulation triangulation(points.size() - 3, points.size() - 2, points.size() - 1, delaunay);
		for (unsigned int i = 0; i < (points.size() - 3); i++) triangulation.addPoint(i);
		
		if (!unwantedTriangles) triangulation.deleteUnwantedTriangles();
