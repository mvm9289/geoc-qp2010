...
void exer4execute()
{
	if (points.size() > 0)
	{
		...
		Triangulation triangulation(triangleContainer.a, triangleContainer.b, triangleContainer.c);
		for(unsigned int i = 0; i < (points.size() - 3); i++) triangulation.add(points[i]);
		
		polygons = triangulation.getTriangles();
	}
}
